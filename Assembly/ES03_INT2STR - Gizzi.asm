...
;supporti numeri a 8bit e stampi numeri fino a 255 
...
data segment
; add your data here!
num DB 255 ; Numero a due cifre da stampare
ends
stack segment
dw 128 dup(0)
ends
...
MOV BL, 100 ; Divisore a 8 bit per separare centinaia e decine
DIV BL ; AL -> quoziente (centinaia), AH -> resto (decine)
MOV BL, AH ; Salva in BL il resto
; Stampa la decina
ADD AL, '0' ; Converte la centinaia in carattere ASCII
...
; Stampa l'unita'
MOV AL, BL ; Carica il resto (decina) in AL 
MOV BH,BL ;Salva la decina in BH
MOV BL, 10 ;divisore per separare decine e unita'
DIV BL ;AL ->quoziente (decine), AH -> resto (unita')  
MOV DH, AH ;salvo il resto in dh
ADD AL, '0' ; Converte l'unità in carattere ASCII
MOV DL, AL ; Carica il carattere nel registro DL per la stampa
MOV AH, 02H ; Funzione DOS per stampare un carattere
INT 21H ; Interruzione per eseguire la stampa
MOV DL, DH ;Carico il resto nel registro DL per la stampa
ADD DH, '0';converto l'unita' in carattere ASCII
