...
data segment
    ...
    error_sott db " ERRORE! impossibile sottrare per un numero maggiore del primo operatore. $"
    errore_rand db "ERRORE! inserire i parametri in modo crescente. $" 
...
divisione endp
;--------------------------
;Procedura per generare un numero casuale tra l'op1 e l'op2   
random proc

xor dh,dh;azzero dh

salvataggio:
mov ah, 2Ch ;Interrupt per ottenere l'ora di sistema
int 21h 

xor dh,dh
;controllo se gli operatori sono stati messi in ordine sbagliato 
mov ax, op1
mov bx, op2  
cmp ax,bx 
jg errore3 

;controllo se in DL (centesimi di secondo) ho un valore compreso tra 1 e 10
cmp dx, op1 
jl salvataggio
cmp dx, op2    
jg salvataggio
              
mov result, dx ;salvo il risultato da DX 
jmp fine5
...
random endp
;---------------------------- 
;Procedura per stampare i primi 5 numeri della sequenza di fibonacci
fibonacci proc
...
mov bh, 0     ; Primo numero di Fibonacci
mov bl, 1     ; Secondo numero di Fibonacci

ciclo:
add bh, bl    ; Calcolo il numero

;Stampa del numero con conversione del carattere 
mov dl, bh    
add dl, '0'   
mov ah, 2     
int 21h       
...
mov al, bl    ; Salvo il valore di bl in al temporaneamente
mov bl, bh    ; Aggiorno il secondo numero con il nuovo numero di Fibonacci
mov bh, al    ; Aggiorno il primo numero con il vecchio valore del secondo

loop ciclo    ;Ripeto il ciclo
